class Post {
  constructor(_base, _preview, _postcounter, _option = {}) {
    ...
    this.preview = _preview
      ? document.querySelector(_preview)
      : document.querySelector(".preview")
    this.postcounter = _postcounter
      ? document.querySelector(_postcounter)
      : document.querySelector(".postcounter")
    this.option = _option
  ...
  setup() {
    this.base.addEventListener("keypress", (e) => {
      ...
      setTimeout(() => {
        this.countPost(this.base.value)
      }, 200)
      if (e.key === "Enter") {
        // console.log(e)
        e.preventDefault()
...
class Preload {
  ...
  setup() {
    document.body.onload = () => {
      setTimeout(() => {
        this.preload.style.marginTop = `calc(-100vh - 300px)`
      }, 2000)
      setTimeout(() => {
